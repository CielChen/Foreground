...
/*	scene=cvLoadImage("G:\\Code-Foreground\\Data\\Differ\\example_scene.jpg"); //载入场景图像
	back=cvLoadImage("G:\\Code-Foreground\\Data\\Differ\\example_background.jpg");  //背景图像
*/	
	scene=cvLoadImage("G:\\Code-Foreground\\Data\\Scene\\20170228111043_scene.jpg"); //载入场景图像
	back=cvLoadImage("G:\\Code-Foreground\\Data\\Background\\20170228111043_back.jpg");  //背景图像
	...
	for(int i=0;i<fore->height;i++)
	{
		for(int j=0;j<fore->width;j++)
		{
			...
			if(similar<5000)  //相似度小于5000，则认为两个像素点相同，那么就将该点判定为背景（黄色）
			{ 
...
//	cvSaveImage("G:\\Code-Foreground\\Data\\DifferRGB\\Foreground\\example_foreground.jpg", fore);  //保存前景图片 
	cvSaveImage("G:\\Code-Foreground\\Data\\DifferRGB\\Foreground\\20170228111043_fore.jpg", fore);  //保存前景图片 
